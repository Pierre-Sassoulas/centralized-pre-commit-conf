...
def main(argv=None):
    ...
    parser.add_argument("--url", default=config["DEFAULT_REPOSITORY"].get(), help="Git repository URL")
    parser.add_argument("--branch", default=config["DEFAULT_BRANCH"].get("str"), help="Git branch")
    parser.add_argument("--path", default=config["DEFAULT_PATH"].get(), help="Path inside the git repository")
    ...
    config_files = config["CONFIG_FILES"].get()
    if args.verbose:
